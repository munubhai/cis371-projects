...
const onGoing: Array<OngoingCourse> = [
    {
        ...
        credits: 3,
        classTimes: ["Mon 12-12:50", "Wed 12-12:50", "Fri 12-12:50"]
    },
    {
        code: "CIS 371",
        name: "Web Application Development",
        credits: 3,
        classTimes: ["Mon 2-2:50", "Wed 2-2:50", "Fri 2-2:50"]
    },
    {
        code: "CIS 351",
        name: "Computer Organization and Assembly Language",
        credits: 4,
        classTimes: ["Mon 3-3:50", "Wed 3-3:50", "Fri 3-3:50"]
    },
    {
        code: "CIS 290",
        name: "Professional Responsibilities and Practices",
        credits: 3,
        classTimes: ["Tue 10-11:15", "Thu 10-11:15"]
    }
]
...
for (let i in onGoing) {
    ...
    course.innerText = onGoing[i].code + " " + onGoing[i].name + " (" + onGoing[i].credits + " credits)";
    listCourseProgress.appendChild(course);

    const times = document.createElement("ul");
    course.appendChild(times);

    for (let j in onGoing[i].classTimes) {
        const time = document.createElement("li");
        time.innerText = onGoing[i].classTimes[j];
...
for (let i in completed) {
    totalCredits += completed[i].credits;
    ...
    codeName.innerText = completed[i].code + " " + completed[i].name;
    credits.innerText = completed[i].credits.toString();
    semesterCompleted.innerText = completed[i].semesterCompleted;
